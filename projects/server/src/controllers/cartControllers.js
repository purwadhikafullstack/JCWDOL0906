...
module.exports = {
    getAll: async (req, res) => {
        ...
        try {
            const [a] = await sequelize.query(`SELECT * FROM carts c JOIN products p ON c.product_id = p.id LIMIT ${limit} OFFSET ${offset}`)
    ...
    addToCart: async (req, res) => {
        ...
        try {
            ...
            if (!isExists) {
                ...
                await cart.create({
                    user_id, product_id, qty, price, total_price
                })
                res.status(200).json(successResponse("add to cart success", null, null))
            } else {
                ...
                res.status(200).json(successResponse("update product success", null, null))
            }
        } catch (error) {
            console.log(error)
            res.status(500).json(failedResponse(error))
    ...
    updateCart: async (req, res) => {
        ...
        try {
            ...
            let qty = 0
            if (req.body.method === 'plus') { qty = Number(dataValues.qty) + 1 } else {
