    ...
    try {
      ...
      if (action === "confirm") {
        if (status[0].status === "Dibatalkan")
          return res
            .status(400)
            .json(failedResponse("Transaksi tidak bisa dikonfirmasi"));
        if (status[0].status !== "Menunggu Konfirmasi")
          return res
            .status(400)
            .json(failedResponse("Status transaksi sudah terkonfirmasi"));
        const data = await transaction.update(
          { status: "Diproses" },
          {
            where: {
        ...
        res
          .status(200)
          .json(
            successResponse("Success, transaction already confirmed", "", "")
          );
      } else {
        if (dataValues.status !== "Menunggu Konfirmasi")
          return res
            .status(400)
            .json(failedResponse("Transaksi tidak bisa dibatalkan"));
        const data = await transaction.update(
        ...
        res
          .status(200)
          .json(successResponse("Success, transaction rejected", "", ""));
      }
    } catch (error) {
