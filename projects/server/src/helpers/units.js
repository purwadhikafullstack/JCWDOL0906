...
exports.addStock = async (product_id, qty) => {
    ...
    try {
        ...
        if (data_stock) {
            ...
            await stock.update({ default_unit_id: exists_default_unit_id, conversion_unit_id: exists_conversion_unit_id, default_unit_qty: update_default_stock, conversion_unit_qty: update_conversion_stock }, {
                where: {
                    product_id: product_id
                }
            })
...
exports.recalculateStock = async (product_id, qty) => {
    ...
    try {
        ...
        recalculateStockDefault = (data.dataValues.conversion_unit_qty / qty) * (data_stock.dataValues.default_unit_qty === 0 ? 1 : data_stock.dataValues.default_unit_qty)
        ...
        await stock.update({ default_unit_qty: recalculateStockDefault }, {
            where: {
                product_id: product_id
            }
        })
